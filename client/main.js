...
function handleSubmit(e) {
	...
	if (!name || name.replace(/\s*/g,'') === '') {
		showAlert('.alert-error', '이름이 없습니다~~~', 2000);			// node 값을 '.alert'로 주어도 한 클래스만 적용이되는데 querySelector는 가장 첫번째롤 만나는 class만 선택 되기 때문에 두번째에 있는 '.alert-success'는 활성화가 되지 않는다.
		
		shake.restart();	//	부르르 함수 (gsap)
	...
	clearContents(resultArea);	// input 값을 비워주는 함수
	insertLast(resultArea, pick);	// 아래에 input값을 넣어서 주접을 보여준다.
}

//& 과제
// 이름을 제대로 입력했을때 클립보드에 복사가 될 수 있도록.
// let state = false;
// state = true;
// if(state) { ... logic}
// 상태 값이 정상이라면 로직이 실행될수 있게
